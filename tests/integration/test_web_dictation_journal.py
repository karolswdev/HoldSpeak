...
def test_dictation_page_includes_journal_tab(test_client: TestClient) -> None:
    ...
    assert '{ id: "journal", label: "Journal" }' in shell
    assert "/api/dictation/journal" in shell
    # HS-132-12: HS-117-08 moved the journal ledger itself into
    # cores/dictation/Journal.tsx; the search + clear verbs live there.
    journal = (
        Path(__file__).resolve().parents[2]
        / "web/src/pages/cores/dictation/Journal.tsx"
    ).read_text()
    assert "Search the journal" in journal and "Clear all?" in journal
...
def test_dictation_journal_premium_and_a11y_markers(test_client: TestClient) -> None:
    ...
    assert "ConfirmVerb" in journal and "SurfaceState" in journal
    assert "prefers-reduced-motion" in css
    assert ".innerHTML" not in journal
