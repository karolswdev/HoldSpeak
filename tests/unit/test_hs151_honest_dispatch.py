...
def _isolated_kernel_database(tmp_path: Any, monkeypatch: Any):
    """Keep the real external-egress path off the owner's database.

    These tests intentionally exercise ``run_external_egress``, which lazily
    composes the process-global kernel broker.  Without resetting both database
    and broker singletons, a unit test opens and reconciles the default
