...
#
# Lane 1 (parallel, ~4 min): everything except the serial tail, under plain
#   xdist `load`. No pytest-timeout here: its thread method kills the whole
#   worker process on a hang ("node down"), which cascades into xdist
#   scheduler crashes — a hang in this lane surfaces as wall-clock, not as
#   dead workers lying about results.
