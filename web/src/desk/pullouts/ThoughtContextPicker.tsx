...
export function ThoughtContextPicker({
  ...
  const candidateRows = (items: ThoughtContextCandidate[]) => (
    <ul className="thought-context-picker-list">
      {items.map((row) => {
        ...
        const noteCount = row.kind === "knowledge" ? `${row.leaf_count} note${row.leaf_count === 1 ? "" : "s"}` : "Note";
        const state = row.selected ? "Attached" : row.disabled_reason || noteCount;
        return <li key={row.ref}><button
          ref={(node) => { if (node) rowRefs.current.set(row.ref, node); else rowRefs.current.delete(row.ref); }}
          type="button" className="thought-context-choice" disabled={disabled}
  ...
  const picker = <section ref={dialogRef} className="thought-context-picker" role="region" aria-label="Attach context" tabIndex={-1} style={desktopPosition || undefined}>
    <header className="thought-context-picker-head"><div><strong>Attach context</strong><small>Choose what AI may use for this Thought.</small></div><button type="button" className="thought-context-close" aria-label="Close" title="Close" onClick={onClose}>×</button></header>
    <div className="thought-context-picker-group thought-context-policy-group"><h3>On this Thought</h3>
      {!attachments.length ? <><p className="thought-context-empty">None</p><p className="thought-context-empty">Attach context to use it by default.</p></> : <ul className="thought-context-picker-list">{attachments.map((attachment) => <li key={attachment.ref} className="thought-context-policy-row">
        <span><span className="thought-context-choice-title">{attachment.title}</span>{attachment.is_default ? <small className="thought-context-default-marker">Default</small> : null}</span>
        <button type="button" className="thought-context-row-action" aria-label="Remove from this Thought" disabled={Boolean(pending)} onClick={() => void remove(attachment)}>{pending === attachment.ref ? "Removing…" : "Remove"}</button>
      </li>)}</ul>}
      {attachments.length ? <button type="button" className="thought-context-policy-action" aria-label="Use these by default" disabled={Boolean(pending || !defaultContext)} onClick={() => void replaceDefault(attachments.map((item) => item.ref), "use")}>{pending === "default:use" ? "Saving…" : "Use for new Thoughts"}</button> : null}
    </div>
    <div className="thought-context-picker-group thought-context-policy-group"><h3>For new Thoughts</h3>
      {!defaultContext?.selections.length ? <p className="thought-context-empty">None</p> : <ul className="thought-context-picker-list">{defaultContext.selections.map((selection) => <li key={selection.ref} className="thought-context-policy-row"><span className="thought-context-choice-title">{selection.title}</span><span className="thought-context-choice-meta">{selection.state === "current" ? `${selection.leaf_count} note${selection.leaf_count === 1 ? "" : "s"}` : "Unavailable"}</span></li>)}</ul>}
      {defaultContext?.selections.length ? <button type="button" className="thought-context-policy-action" disabled={Boolean(pending)} onClick={() => void replaceDefault([], "stop")}>{pending === "default:stop" ? "Stopping…" : "Stop using by default"}</button> : null}
    </div>
    ...
    {!searching && view === "compact" ? <>
      ...
      <button type="button" className="desk-chip quiet thought-context-browse" onClick={() => setView("browse")}>Browse all notes</button>
      ...
      {nextCursor ? <button type="button" className="desk-chip quiet thought-context-more-results" disabled={loading} onClick={() => void load(nextCursor)}>{loading ? "Loading…" : "Show more"}</button> : null}
      {!searching ? <button type="button" className="desk-chip quiet" onClick={() => setView("compact")}>Back</button> : null}
    </div>}
  </section>;
