...
export const STEP_PALETTE: Array<{ label: string; make: () => LinearStep }> = [
  ...
  {
    label: "Decisions",
    make: () => ({ kind: "extract", artifactType: "decisions" }),
  },
  {
    label: "Action items",
    make: () => ({ kind: "extract", artifactType: "action_items" }),
  },
...
export function stepLabel(s: LinearStep): string {
  switch (s.kind) {
    case "summarize":
      return "Summarize";
    case "extract":
      return `Extract · ${s.artifactType.replace(/_/g, " ")}`;
    case "rewrite":
      return `Rewrite · ${s.tone}`;
    case "keepIf":
      return `Keep if · ${s.keyword}`;
    case "llm":
      return s.prompt
        ? `LLM · “${s.prompt.slice(0, 40)}${s.prompt.length > 40 ? "…" : ""}”`
        : "LLM prompt";
  }
}
...
function nodeKind(s: LinearStep): Record<string, Record<string, unknown>> {
  switch (s.kind) {
    case "summarize":
      return { summarize: {} };
    case "extract":
      return { extract: { _0: s.artifactType } };
    case "rewrite":
      return { rewrite: { tone: s.tone } };
    // Swift's `keepIf` case name reaches the wire snake_cased by the canonical coder.
    case "keepIf":
      return { keep_if: { keyword: s.keyword } };
    case "llm":
      return {
        llm: { name: "LLM call", prompt: s.prompt.trim() || "{input}" },
      };
  }
}
...
export function parseLinearGraph(graph: unknown): LinearStep[] | null {
  ...
  if (!g || !Array.isArray(g.nodes) || !Array.isArray(g.exec_edges))
    return null;
  ...
  while (cursor) {
    ...
    switch (tag) {
      ...
      case "extract":
        steps.push({
          kind: "extract",
          artifactType: String(payload._0 || "decisions"),
        });
        break;
      case "rewrite":
        steps.push({
          kind: "rewrite",
          tone: String(payload.tone || "Executive"),
        });
        break;
      case "keep_if":
