...
type ScheduleStub = { id: string; title: string };

let storeState: {
  recording: "idle" | "busy" | "recording";
  items: { meeting: Meeting[] };
  scheduledRecordings: ScheduleStub[];
} = {
  recording: "idle",
  items: { meeting: [] },
  scheduledRecordings: [],
...
function makeSchedule(): ScheduleStub {
  return { id: "schedule-only", title: "Door-only schedule" };
}
...
describe("MeetingsLane", () => {
  ...
  // Schedule rows belong to the Door aggregate, never this live/recent lane.
  it("does not create a Meetings lane for a schedule-only store", () => {
    // The component deliberately selects no schedule state. A store that knows
    // about a future schedule must therefore leave this lane absent.
    resetStore({ scheduledRecordings: [makeSchedule()] });
    const { container } = renderLane();
    expect(container.innerHTML).toBe("");
  });
