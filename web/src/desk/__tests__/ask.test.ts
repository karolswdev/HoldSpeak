...
import { ASK_LENSES, askContexts, askLineageLine, keepAsk, runAsk } from "../ask";
...
const items = {
  ...
  kb: [], recipe: [], chain: [], workflow: [], directory: [], coder: [],
} as unknown as Items;
...
describe("the lineage line (iPad grammar)", () => {
  ...
  it("a single card is named", () => {
    expect(askLineageLine(ctx.slice(0, 1), "Summarize")).toBe("Q3 kickoff → Summarize");
  });
});
...
describe("the run/keep wire", () => {
    ...
    vi.stubGlobal("fetch", (url: string, init: any) => {
      ...
      return Promise.resolve({
        ...
        json: () => Promise.resolve({
          output: "PRINTED", egress: { scope: "cloud", host: "192.168.1.43" },
          model: "Qwen3.5-9B-Q6_K", profile_id: "p1",
        }),
      });
    });
    const r = await runAsk({
      prompt: "Go", lens: "Distill",
      context: askContexts(items, ["m1", "n1"]), profileId: "p1",
    });
    expect(sent).toEqual({
      prompt: "Go", lens: "Distill",
      context: [
  ...
  it("keepAsk posts the /api/ask/keep payload (id+title per card) and returns the artifact id", async () => {
    ...
    vi.stubGlobal("fetch", (url: string, init: any) => {
      ...
      sent = JSON.parse(init.body);
      return Promise.resolve({ ok: true, json: () => Promise.resolve({ artifact_id: "artifact_x" }) });
    });
    const id = await keepAsk({
      lens: "Distill", prompt: "Go", output: "PRINTED",
  ...
  it("a hub error surfaces, never throws", async () => {
    vi.stubGlobal("fetch", () => Promise.resolve({
      ok: false, status: 502, json: () => Promise.resolve({ error: "no model" }),
    }));
...
describe("the lens presets", () => {
  it("mirror the iPad RouteLenses (five, Summarize first)", () => {
    expect(ASK_LENSES.map((l) => l.name)).toEqual([
      "Summarize", "Action items", "Risks", "Decisions", "Draft email",
    ]);
  });
});
