...
function SettingsFace({ hero, scope }: CoreProps) {
  ...
  const [calSources, setCalSources] = useState<{
    sources: Array<{
      ...
      host: string | null; calendar_count: number;
      last_read: string | null; egress: boolean;
    }>;
    auto_record: string;
                ...
                {sources.map((entry, index) => {
                  ...
                  return (
                    <SurfaceLedgerRow
                      ...
                      cells={
                        <span style={{ display: "inline-flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                          ...
                          {host
                            ...
                            : <span className="surface-token" data-chip>THIS DEVICE</span>}
                          {(() => { const ct = stat ? countToken(stat.calendar_count, "CALENDAR", "CALENDARS") : null; return ct ? <span className="surface-token" data-chip>{ct}</span> : null; })()}
                          {stat?.last_read ? <span className="surface-token" data-chip>LAST READ {stat.last_read}</span> : null}
                        </span>
                      }
                      trailing={
                        <>
                      ...
                      onToggle={() => (editing ? closeCalWell() : openCalWell(entry.id, entry.url))}
                    >
                  ...
                  trailing={
                    calWellFor !== "new" ? (
                      <>
                        ...
                        <Button variant="ghost" dense onClick={(e) => {
                          e.stopPropagation();
