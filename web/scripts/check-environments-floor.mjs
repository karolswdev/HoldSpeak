...
try {
  ...
  page.on("request", (request) => {
    ...
    const responses = {
      ...
      "/api/settings/hub": {
        ...
        rhythm: { loops: 0 }, sounds: { on: false },
        system: { host: "THIS DEVICE", mesh: false, remote: false },
        posture: "neutral", writtenAt: null,
      },
      "/api/state": {
      ...
      "/api/thoughts": { items: [], next_cursor: null },
      "/api/desk/needs-you": { count: 0, mutedCount: 0, projects: [], items: [] },
  ...
  if (settleWalk) {
    ...
    await page.waitForFunction(() =>
      [...document.querySelectorAll(".surface-ledger-row")].some((row) =>
        row.querySelector(".surface-ledger-primary")?.textContent === "Wallpaper",
      ),
    );
    await page.evaluate(() =>
      [...document.querySelectorAll(".surface-ledger-row")]
        .find((row) => row.querySelector(".surface-ledger-primary")?.textContent === "Wallpaper")
