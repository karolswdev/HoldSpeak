"""Atlassian CLI (acli) Jira connector pack.

HS-166-01. Read-only ``acli jira`` command policy + manifest for the
V0 Jira provider adapter.  Mirrors ``github_cli.py``'s idiom — the
allowlist is the source of truth, and anything not on it is rejected
before exec.

The acli CLI shapes commands as ``acli jira <group> <verb>`` — the
binary is ``acli`` and the first positional is always ``jira`` (the
product selector).  The allowlist therefore uses **3-tuples**
``(product, group, verb)`` and the command validator expects at least
four tokens: ``[acli, jira, <group>, <verb>, ...]``.

The legacy ``jira_cli.py`` (go-jira enrichment) is PARKED, untouched
(owner ruling: never delete).
"""
...
CONNECTOR_ID = "acli_jira"

# ── Allowlist ────────────────────────────────────────────────────────
# 3-tuples: (product, group, verb).  The command shape is
# ``acli jira <group> <verb> [flags...]``, so ``argv[1:]`` must
# start with one of these prefixes.
#
# Read-only verbs only.  ``auth login``, ``auth logout``,
# ``workitem create|edit|delete|transition`` and every other
# write verb are rejected.
...
MANIFEST: ConnectorManifest = validate_manifest(
    {
        "id": CONNECTOR_ID,
        "label": "Atlassian CLI (Jira)",
        "version": "0.1.0",
        "kind": "cli_enrichment",
        "capabilities": ["commands"],
        "description": (
            "Read-only ``acli jira`` calls: auth status/switch, project "
            "list/view, workitem search/view. No writes, no transitions, "
            "no token management. The owner authenticates via "
            "``acli jira auth login``; HoldSpeak never stores credentials."
        ),
        "requires_cli": "acli",
