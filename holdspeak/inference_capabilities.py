...
def _result_schema(operation: str, kind: str, contract: str) -> dict[str, Any]:
    ...
    schemas: dict[str, dict[str, Any]] = {
        ...
        "meeting_title": _closed_object_schema(
            properties={"title": scalar}, required=("title",)
        ),
        ...
        "rails_summary": _closed_object_schema(
            properties={"summary": scalar}, required=("summary",)
        ),
        "cadence_draft": _closed_object_schema(
            properties={"draft": scalar}, required=("draft",)
        ),
        "decision_promotion_draft": _closed_object_schema(
            properties={"draft": scalar}, required=("draft",)
        ),
        "pull_request_review_draft": _closed_object_schema(
            properties={"draft": scalar}, required=("draft",)
        ),
    }
...
def builtin_capability_definitions() -> tuple[InferenceCapabilityDefinition, ...]:
    ...
    return (
        ...
        _capability("project.update_draft", "Project update draft", *agents, "Draft a project update from the room's evidence inventory.", operation="project.update.draft", output_kind="project_update_draft", minimum_context_tokens=4096, fallback_dispositions=text_fallback, source_module="holdspeak.services.project_update_service"),
        _capability("project.brief_prepare", "Project preparation brief", *agents, "Draft a bounded preparation brief from a Project's read sources and carried decisions.", operation="project.brief.prepare", output_kind="project_preparation_brief", minimum_context_tokens=4096, fallback_dispositions=text_fallback, source_module="holdspeak.services.preparation_brief_service"),
        _capability("background.rails_summary", "Rails summary", *background, "Summarize an admitted software-delivery event batch.", operation="background.rails.summary", output_kind="rails_summary", minimum_context_tokens=4096, policy="retry.background.standard", fallback_dispositions=text_fallback, source_module="holdspeak.rails_observer", revision=2),
        _capability("background.cadence_draft", "Cadence draft", *background, "Draft one bounded cadence action.", operation="background.cadence.draft", output_kind="cadence_draft", minimum_context_tokens=4096, policy="retry.background.standard", fallback_dispositions=text_fallback, source_module="holdspeak.services.cadence_service", revision=2),
        _capability("decision.promotion_draft", "Decision promotion draft", *background, "Draft a decision-record promotion.", operation="decision.promotion.draft", output_kind="decision_promotion_draft", minimum_context_tokens=4096, policy="retry.background.standard", fallback_dispositions=text_fallback, source_module="holdspeak.services.decision_lifecycle_service", revision=2),
        _capability("delivery.pr_review_draft", "Pull request review draft", *background, "Draft an admitted pull-request review.", operation="delivery.pr.review.draft", output_kind="pull_request_review_draft", minimum_context_tokens=8192, policy="retry.background.standard", fallback_dispositions=text_fallback, source_module="holdspeak.web.routes.delivery_prs", revision=2),
        _capability("calendar.snapshot_extract", "Calendar snapshot extraction", *background, "Extract calendar events from a screenshot via vision.", operation="calendar.snapshot.extract", input_modalities=("text", "image"), output_kind="calendar_snapshot_extraction", vision=True, structured_output=True, minimum_context_tokens=4096, policy="retry.structured.standard", fallback_dispositions=structured_fallback, source_module="holdspeak.services.calendar_snapshot_service"),
