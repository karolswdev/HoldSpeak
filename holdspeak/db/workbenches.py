...
class WorkbenchItemRepository(BaseRepository):
        ...
        with self._connection() as conn:
            ...
            conn.execute(
                ...
                ON CONFLICT(id) DO UPDATE SET
                    ...
                    result_artifact_id = COALESCE(excluded.result_artifact_id, workbench_items.result_artifact_id),
    ...
    def _row(self, row: Any) -> WorkbenchItemRecord:
        return WorkbenchItemRecord(
            ...
            result_artifact_id=row["result_artifact_id"] if "result_artifact_id" in row.keys() else None,
            mint_attempted=bool(row["mint_attempted"]) if "mint_attempted" in row.keys() else False,
...
class WorkbenchRunRepository(BaseRepository):
    ...
    def _row(self, row: Any) -> WorkbenchRunRecord:
        return WorkbenchRunRecord(
            ...
            mint_failures=row["mint_failures"] if "mint_failures" in row.keys() else 0,
