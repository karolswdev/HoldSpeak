...
class RunLifecycle:
    ...
    def __init__(
        self, db: Any, invocation_id: str, definition_ref: str,
        *, operation_id: str = "", broker: Any = None,
    ) -> None:
        self.db = db
    ...
    def begin(
        ...
        *,
        ...
        raw_refs = body.get("grounding_refs", [])
        grounding: list[dict[str, str]] = []
        if isinstance(raw_refs, list):
            revisions = body.get("grounding_revisions")
            ...
            for item in raw_refs:
                if isinstance(item, dict):
                    ...
                    grounding.append(
                        {"ref": str(item).strip(),
                         "revision": str(revisions.get(str(item)) or "unversioned")}
                    )
        source_ref = str(body.get("source_ref") or "").strip()
        if source_ref:
            source_kind = canonical_source_type(body.get("source_type") or "input") or "input"
            grounded_ref = source_ref if ":" in source_ref else f"{source_kind}:{source_ref}"
            grounding.append(
                {"ref": grounded_ref,
                 "revision": str(body.get("source_revision") or "unversioned")}
            )
        ...
        if principal is not None and definition_ref.startswith("persona:"):
            ...
            handle = broker.submit(
                {
                    "request_schema": 1, "request_id": _new_id("request"),
                    "idempotency_key": invocation_id,
                    "operation": {"name": "inference.run", "version": 1},
                    "target": {}, "arguments": {
                        "invocation_id": invocation_id,
                        "definition_ref": definition_ref,
                        "definition_revision": definition_revision or "unversioned",
                        "grounding_refs": grounding, "requested_target_id": requested,
                        "deadline_at": float(body.get("deadline_at") or time.time() + 300.0),
            ...
            if handle["state"] == "refused":
                raise ValueError(handle["receipt"]["outcome"])
            handle = broker.decide(
                handle["operation_id"], "approve", handle["revision"], principal
            )
            return cls(
                db, invocation_id, definition_ref,
                operation_id=handle["operation_id"], broker=broker,
            )
        db.capability_invocations.begin(
            invocation_id=invocation_id, definition_ref=definition_ref,
            initiator=str(body.get("initiator") or "owner"),
            grounding_refs=[f"{item['ref']}@{item['revision']}" for item in grounding],
            requested_placement=requested, input_snapshot=snapshot,
        )
    ...
    def start_attempt(
        ...
        *,
        ...
        if self.broker is not None and self.operation_id:
            ...
            operation = self.broker.store.operation(self.operation_id)
            identity = str(operation["placement"]).removeprefix("node:")
            self.node_principal = Principal(PrincipalKind.NODE, identity)
            claimed = self.broker.claim(self.node_principal, self.invocation_id)
            if not claimed["operations"] or claimed["operations"][0]["operation_id"] != self.operation_id:
                raise ValueError("inference operation could not be claimed")
    ...
    def fail(
        ...
        *,
        ...
        value = self.db.capability_invocations.finish(
            self.invocation_id, state=state, error=error,
        ).to_dict()
        self._close("failed", f"invocation:{self.invocation_id}")
        return value
    ...
    def succeed(
        ...
        *,
        ...
        self._close("succeeded", result_ref)
        return value

    def cancelled(self) -> Optional[dict[str, Any]]:
        value = self.db.capability_invocations.get(self.invocation_id)
        if value is None or value.state != "cancelled":
            return None
        self._close("refused", f"invocation:{self.invocation_id}")
        return value.to_dict()

    def _close(self, outcome: str, result_ref: str) -> None:
        if self.broker is None or self.node_principal is None or not self.operation_id:
            return
        if self.broker.store.receipt(self.operation_id) is None:
            self.broker.receipt(self.operation_id, outcome, result_ref, self.node_principal)
