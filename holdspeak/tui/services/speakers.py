...
def get_known_speakers_map() -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        for speaker in db.meetings.get_all_speakers()
    }
...
def get_speaker_profile_data(speaker_id: str) -> SpeakerProfileData | None:
    ...
    return SpeakerProfileData(
        ...
        meeting_groups=db.meetings.get_speaker_segments(speaker_id),
    )
...
def update_speaker_identity(speaker_id: str, new_name: str, new_avatar: str) -> tuple[bool, bool]:
    ...
    db = db_module.get_database()
    name_success = db.meetings.update_speaker_name(speaker_id, new_name)
    avatar_success = db.meetings.update_speaker_avatar(speaker_id, new_avatar)
