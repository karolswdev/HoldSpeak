...
| `meeting.import` | Meeting import | Turn audio or a transcript file into a normal persisted meeting that can be searched, exported, and analyzed. | experimental | user | source_and_assertions_inspected | no | possible |
| `meeting.intelligence` | Meeting intelligence chain | Run admitted profile-selected analysis plugins over a saved meeting and persist typed artifacts with lineage. | experimental | user | source_and_assertions_inspected | no | possible |
...
| `model.assignment` | Inference assignment | Store sparse owner assignments for capability groups without executing providers or selecting fallback at run time. | internal | operator | source_and_assertions_inspected | no | local |
