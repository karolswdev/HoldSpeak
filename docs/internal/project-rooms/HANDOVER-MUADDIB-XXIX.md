...
Read this, then the running memory, then `.tmp/astra-lane-philo-7-03.out` (Astra's lane on 03). If it is done: check on built (Muad'Dib), pay rounds, merge on the fast jobs, then launch 04 from `story-04-file-it-and-find-it-without-the-repo.md` with the brief's launch setup verbatim. Worktrees to expect: `../wt-philo-7-03` (Astra's). Everything else from today is removed.
