"""Audit the six retained integrated round-two rig observations; no hub or owner data."""
import hashlib
import json
...
shots = root / "pm/roadmap/holdspeak-philo/phase-3-the-meeting-loop/assets/story-02-shots/rig/round-2-integrated"
...
for path in paths:
    ...
    assert provenance["frontend_build"]["index_sha256"] == "1cd6b691cb0828d1c31bd5bfa97507272c5d47ce884d633a646423a34911fcac"
    ...
    rows.append({"case": pair[0], "width": pair[1], "run": data["run_id"], "meeting_id": meeting["id"], "words": meeting["transcriptWords"], "first_satisfied_s": data["terminal_outcome"]["first_satisfied_at_s"], "queue_counts": counts, "raw_target_all_owned": after["hit_test"]["all_owned"], "artifacts": artifacts})
...
(proof / "integrated-observations-index.json").write_text(json.dumps(rows, indent=2) + "\n")
print("Six integrated actual case/width pairs pass; all shots retained; import completion, single Run, closed setup window, plain causes, settled counts and real ready-engine identity verified.")
...
print("Ready-state target hit ownership is retained in the index; inherited aftercare obstruction is ledgered, not waived as whole-face compliance.")
