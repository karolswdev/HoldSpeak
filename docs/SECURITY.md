...
| **Calendar ICS subscription** (`calendar_ingest_conductor.py`) | Owner configures an HTTPS Calendar subscription; refresh runs at boot and every 15 minutes | A bounded ICS fetch to the configured URL. No credentials, caller-supplied headers, cookies, proxy configuration, or redirect follow-up. | HTTPS only; redirects refuse. A local file subscription causes no network egress. |
