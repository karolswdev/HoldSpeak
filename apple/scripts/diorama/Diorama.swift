...
struct Stage: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                Companion(landed: landed, excited: selected != nil).position(x: w * 0.5, y: h * 0.9)
    ...
    private func runDemo() {
        Task { @MainActor in
            while true {
                try? await Task.sleep(nanoseconds: 2_600_000_000); select("cassette")
                try? await Task.sleep(nanoseconds: 3_400_000_000); select(nil)
