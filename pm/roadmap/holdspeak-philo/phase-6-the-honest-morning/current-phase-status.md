...
- [x] 1. Each repair's fence is red on a `git archive origin/main` copy with the branch's tests overlaid, and green on the built branch; both transports where the seam has two (HTTP and `op`). "Unknown tool" or a missing symbol is never counted as red. **(flipped by Muad'Dib 2026-09-25: per story — 01 rendered `Expected: FAILED Received: SAVED` + the IMPORT well + the temp-name cause; 02 producer/rendered/follow-through/advancing-clock reds; 03 zero-token 3 failed + placement `8 failed | 10 passed`; 04 two observer rows; 05 the blank-frame red — retained under `docs/internal/philo/phase-6/{badge,brief,toast,rows,body}/`.)**
...
- 2026-09-25 — exit 3 flipped on the closing brief's rule (1, 2 and 4 PASS; the rehearsal honestly recorded as the inherited driver block: tool selection in attempts 1–2, reconciliation in attempt 3). The 393 glass gap (receipt, caption, badge below the capture bar in the case's shot) is disclosed, not hidden; the orchestrator may overrule the flip. BACKLOG "PHILO-6 follow-ups" row 2 (reconciliation, assigned before exit 3) is NOT paid — Muad'Dib's Fedaykin.
