...
out = Path('.tmp/philo504-receipt-red')
...
hub = gw.Hub(home, token=driver.TOKEN, producer_clock=True,
             record_rehearsal=True, transcript_path=home / 'receipt-red.jsonl').start()
try:
    ...
    with sync_playwright() as play:
        ...
        try:
            driver._reopen_stage(pages, hub, out, 'brief', 'intelligence:desk')
        except RuntimeError as error:
            driver._write_text(out / 'fence-red.txt', str(error) + '\n')
            observed = json.loads((out / 'observations/brief.json').read_text())
            assert len(observed['pages']) == 2
            assert all(p['receipt_present'] is False for p in observed['pages'])
            assert all(p['row_texts'] for p in observed['pages'])
            print('EXPECTED RED: both widths show the saved Codex brief but lack its receipt.')
        else:
            raise AssertionError('pre-fix receipt unexpectedly passed')
        finally:
            browser.close()
finally:
    hub.stop()
