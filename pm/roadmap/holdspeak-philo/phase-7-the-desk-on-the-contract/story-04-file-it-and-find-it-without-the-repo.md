...
- [x] The claim is worded exactly as R6 reworded R3 for the Claude client (above: "a cold-context Claude session (an empty scratch dir, a scratch HOME with only the auth file, no user or project instructions, `--strict-mcp-config` with the one holdspeak server, no preamble), ZERO repository reads in the retained log, discovery from the catalogue alone"), in the evidence and the rehearsal record; R3's qualification intact (the claim is observational); no record calls it a sandbox or says repository access was unavailable.
- [x] The requests are ordinary words; no operation name, argument, id or test clock appears in them.
- [x] The launch setup is used for the Claude client (R6) and its initial context retained: the working root is empty and outside every checkout; no `CLAUDE.md`/`AGENTS.md`/`.claude/`/`.codex/`/`.git` in it or its parents (checked up to `/`); the scratch HOME holds only the access-token credentials file `.claude/.credentials.json` (no settings, memory or instructions; the init event and the retained initial context show none loaded); `--strict-mcp-config` names the one holdspeak server, pointed at the isolated hub.
- [x] Discovery from `tools/list` alone: the retained stream-json event log of every Claude session shows ZERO reads of repository files, source or roadmap documents (a fence over the retained event log; a holdspeak resource that serves a repository document counts as a read). The fence FAILS on the Phase 5 rehearsal's Codex logs (`decision_thought` 25, `import` 3) and on a real Claude session told to read a repository file. Every tool chosen appears in the session's init tool list.
- [x] The note is filed into the named zone and found again; the decision is on his review list, with its reason in its context; the brief is made; each read back through the contract (`op`), not from the client's own text.
- [x] OWNER leg: the receipts of the filed and decided writes (the admitted set) are read back; one per logical operation, each terminal. The principal on each receipt is the OWNER the transport derived (the loopback sidecar forwards the hub's owner token, `holdspeak/mcp/server.py:144-190`), unchanged.
- [x] AGENT leg: the second session's DESK credential is issued through the real settings route; without the grant the write is refused with a refusal receipt naming `desk_delegation_required` and the note is not filed; with the grant the same request executes and its receipt names the delegation. Both fenced through the real remote route (not the sidecar), each receipt read back through the contract. The agent session also meets the cold-context claim and the zero-read fence.
- [x] Run against an isolated HOME; the effective HOME, lock and DB path retained; never the desk.
- [x] The Desk reopened and shot at 1440 and 393: the note in its zone, the decision in the review rows, the brief. Real-engine and replayed runs labelled separately.
- [ ] Technical rehearsal completes with Muad'Dib's check recorded; the evidence reads "REHEARSED; OWNER REVIEW PENDING", never an observed sitting. — NOT ticked: the rehearsal completed and the check recorded in the tree is the Astra-role check on built (`checks/story-04-built-astra-role-r1.md`, RATIFY-WITH-CONDITIONS, paid); a separate Muad'Dib check file is not in the tree. The evidence reads "REHEARSED; OWNER-REVIEWED", never a sitting.
- [x] The owner reviews the published shots; the review recorded in the tree.
...
- 2026-09-26 — THE OWNER REVIEWED THE RERUN (AskUserQuestion, 2026-09-26 early, on the published page, v2): "Reviewed — it's right". The first closing run had bounced: "the decision is thin". The Astra-role check on built (`checks/story-04-built-astra-role-r1.md`, RATIFY-WITH-CONDITIONS) paid: C1 a read of a repository-document resource (`holdspeak://desk/constitution`) is a zero-read finding (red recorded); C2 `desk.update` "Change a decision … the new text replaces the old"; C3 stale records corrected; C4 boxes 1, 3, 4 reworded to R6's Claude claim; C5 ruled by the coordinator (the commit author header is the repository's git identity). Flipped done.
