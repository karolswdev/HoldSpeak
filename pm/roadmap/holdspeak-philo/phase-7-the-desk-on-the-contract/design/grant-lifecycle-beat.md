...
- **Status:** round four (the last). Astra r1, r2 and r3 (`../checks/lifecycle-beat-astra-r1.md`, `-r2.md`, `-r3.md`), each RATIFY-WITH-CONDITIONS; r2 ratified the round-two payments 3–7 and the desk-only scoping; r3 found the scope proportionate, nothing to cut. Every condition is paid below ("Round two", "Round three" and "Round four" ledgers at the end). Story 02's implementation brief waits for Astra's re-check.
...
|---|---|---|---|
| T1 | Admission refusal (the ordinary no-grant path; also owner-only and capability refusals of `delegation.*`) | `_refuse_attempt`: `create_operation` in state `refused`, then `_terminal` (`broker.py:332-344`) | NEW store method `create_refused_with_receipt(values, outcome)`: ONE connection, `BEGIN IMMEDIATE`, then exactly `create_operation`'s logic (`journal.py:127-155`: the same columns and defaults, incl. `delegator_kind`, `delegator_identity`, `authority_basis`, `target_ref` from provenance; the same `(principal_identity, idempotency_key)` lookup — an existing row is returned with its existing receipt and nothing is written; `idempotency_payload_mismatch` raised as today), then the receipt INSERT, then commit. The seam needs an existing row, so T1 needs this INSERT path. |
...
| T4 | Explicit owner rejection | `transition(..., decision="reject")` + `_terminal` (`broker.py:220-225`) | `transition_and_receipt(..., strict=True, decision="reject")`: the `decision` column is written in the same UPDATE as the state |
...
| T7 | Reaper of `awaiting_execution`/`claimed` | `transition` + `_terminal` (`liveness.py:40-53`) | `transition_and_receipt(..., strict=True, warrant_revoked=1)`; BOTH strict codes (`operation_already_terminal`, `operation_revision_conflict`) = today's `continue` (`liveness.py:46-52`) |
...
- **The seam gains four optional keywords, nothing else:** `strict` (below), `decision` (T4), `warrant_revoked` (T7), `effect: Callable[[conn], None]` (T9). The kernel tables and `kernel_desk_delegations` live in the same database (`db/schema.py:1816`, `:2125`); the precedent writes them in one transaction (`schedule_delegated.py:80-95`).
- **The explicit transaction order (Astra r3 finding 1).** Today the seam reads the receipt BEFORE SQLite opens a transaction (it opens implicitly at the UPDATE, `journal.py:379-388`; Astra traced it). The seam, and `create_refused_with_receipt`, become, on ONE connection:

| Step | Action |
|---|---|
| 1 | `conn.execute("BEGIN IMMEDIATE")` (as `schedule_delegated.py:81`, `schedule_delegation.py:69`): the write lock is held from here to commit |
| 2 | Read the existing receipt and the operation's revision. Default path: an existing receipt → return it; the effect does NOT run again. Strict path: an existing receipt → `operation_already_terminal`; a moved revision → `operation_revision_conflict`; nothing written. Both checks run BEFORE any effect. |
| 3 | `effect(conn)` when given |
| 4 | UPDATE the state, `revision+1`, and `decision` / `warrant_revoked` when given |
| 5 | INSERT the receipt |
| 6 | commit (leaving the `with` block); any exception in 3–5 rolls back 3–5 |

- **The callback contract.** `effect` receives THE seam's connection and runs local SQL on it ONLY: the grant-table UPDATE/INSERT, nothing else. It never calls a service or store method: a nested `db._connection()` on the same thread opens ANOTHER connection (`db/connection.py:137-142`: a busy thread gets a fresh one; Astra probed it), which either commits outside the transaction or blocks on the outer write lock. The routes release the credential-store lock (`principals.py:113`, held inside `issue`/`revoke`) before they enter the kernel: the identity lookup of invariant 5 is a short locked read that returns before `delegation.revoke` is submitted. Fence F24.
...
- **A LIVE grant with no credential (Astra r2 finding 4):** `GET /api/settings/remote` also returns `delegations: [{identity, state, grant_id, expires_at}]` for each grant row that is LIVE in storage and whose identity has no credential row. Its `state` and chip come from the SAME time-aware rule as every other chip (`by_identity` with `now`, check order row 2): a LIVE row past its `expires_at` shows EXPIRED → FILING STOPPED, never ALLOWED from the stored state alone (Astra r3 finding 4). The EXISTING Remote Access ledger renders those rows too: today it renders only when remote access is on and `credentials.length > 0` (`SettingsCore.tsx:612`, `:615`), so a surviving grant would vanish (the branch-removal scar). The condition becomes "any credential OR any delegation row", and delegation rows render whatever the remote switch says (the grant is authority; the owner must be able to stop it). Each such row carries the identity, its chip (FILING ALLOWED, or FILING STOPPED when expired) and, while the grant is not expired, the "Stop filing" verb. No new screen; the look is the canvas's (noted there).
...
| F21b | Boundary B: a fault injected in the `effect` of T9 after the grant-table write → the transaction rolls back: assert SEPARATELY that the grant row is unchanged (LIVE, same `updated_at`, `revocation_reason` empty), that there is no receipt and that the operation is still `claimed`. Then advance the injected clock PAST the warrant's `execution_expires_at` (the reaper returns zero before it, `liveness.py:18-30`; Astra probed both sides) and run the reaper: `indeterminate`, `execution_liveness_expired`, with a receipt. That outcome is the reaper's generic conservative class, not the proof of rollback; the separate grant assertion is. A retry then revokes. | Mutation red: commit the grant write outside the receipt transaction (then REVOKED with no receipt). |
| F23 | Visibility with no credential, API and rendered: remote on, credential issued, grant G1; the credential is removed (self-revoke, and separately TTL cleanup); `GET /api/settings/remote` lists G1 under `delegations`; the Remote Access ledger RENDERS the row with FILING ALLOWED and "Stop filing" at 1440 and 393; clicking it writes the `delegation.revoke` receipt (`owner_revoked`); the row then disappears and the ledger is empty; the receipt is still readable after the last row is gone. Repeat with the remote switch OFF. Expired case: a grant with `expires_at`, credential gone, the injected clock past `expires_at`, the stored state still LIVE → the API says EXPIRED and the rendered row says FILING STOPPED at both widths; an agent write with a reissued credential is refused `desk_delegation_expired`. | Red on main: no `delegations` field and the ledger hidden by `credentials.length > 0` (`SettingsCore.tsx:615`). Mutation red: the orphan row's chip reads the stored `state` (the expired case says ALLOWED). |
| F24 | One connection per atomic write: the fence records the connection ids the real database hands out while a T1, T4 and T9 transaction is open and asserts exactly one; the T4 case also asserts the operation ROW has `decision='reject'` and `state='refused'`, beside its receipt; a T9 fault after the effect leaves no committed grant change. | Mutation red: an `effect` that calls a store method (a second connection id, or a grant change committed outside the rollback); T4 without `decision`. |
...
- **For the canvas (story 02 Preconditions):** the verb and chip strings; how a LIVE grant with no credential row (invariant 5, F23) looks on the existing ledger, including its expired state; and the ledger's caption, which today is the literal `CREDENTIALS` (`SettingsCore.tsx:618`) and must change when it holds rows without credentials.
...
## Round four (Astra r3 conditions → where paid)

| Astra r3 | Where paid |
|---|---|
| Finding 1: explicit transaction order; the callback uses the supplied connection only; the credential lock released before the kernel | Invariant 4 "The explicit transaction order" (steps 1–6) and "The callback contract"; F24 |
| Finding 2: T4's `decision="reject"` inside the transaction; strict before effect; default path never reruns the effect; T7 catches both codes; T1 keeps `create_operation`'s fields, idempotency and provenance on one connection | T4 and T7 rows; T1 row; step 2; the four-keyword line; F24 (row + receipt) |
| Finding 3: F21b past the deadline; the grant asserted separately | F21b |
| Finding 4: the orphan-grant chip is time-aware; F23 extended; the caption for the canvas | Invariant 5 "A LIVE grant with no credential"; F23 expired case; Open (canvas) |
| File the non-desk BACKLOG row at merge | Open (orchestrator) |
