...
OUTCOME: **built — technical corrections verified; usefulness partial**. Muad'Dib's [recorded RATIFY-WITH-CONDITIONS](checks/story-02-built-muaddib.md) is paid by the [round-two response](checks/story-02-built-astra-response.md), awaiting his re-read. Failed and retrying wells now display the ratified **LAST ERROR · PROVIDER FAILED**, with the actual LAST ATTEMPT host. The queue header receives fresh settlement counts. Same-identity summary/transcript wells stay mounted through a desk refresh. Per the owner's ruling, summary is open; transcript is collapsed with its word count and one click to open. Read facts contain no raw IDs. The earlier statement that the queue wrapper retained a plain cause was too broad; the exact words and failing displayed-word fence replace that claim.

PROOF:

- [Round-two report, raw red/green evidence and observation indexes](../../../../docs/internal/philo/phase-3/summary/round-2/report.md). Astra independently reproduced five backend failures and nine rendered failures against `dfbaccd5`, plus the isolated refresh-clearing failure and two old-atlas failures. Final focused Python: **164 collected / 164 passed**. Full web: **307 files / 2,813 tests**, typecheck/build/bundle gate pass.
- Full serial Python, isolated HOME, no metal: **12 failed, 11,559 passed, 116 skipped, 4 xfailed in 5678.77 s (exit 1)**. [Raw output and a/b/c classification](../../../../docs/internal/philo/phase-3/summary/round-2/full-suite-classification.md). Nine failures match the archived-main baseline; three old lane expectations/probes are corrected, with an independent follow-up of 22 passed in 43.18 s (DW `2026-09-23T23:59:47Z`). No full Python green or flake claim.
- Main's A3 is integrated through `5de5d0c3`; both lanes' rig capabilities remain. Actual failed/retrying/ready J6 cases pass at **1440 and 393** on the integrated build: plain cause, settled header counts, summary open and transcript folded. Ready outputs arrive in **6.666 / 7.935 seconds** after one Run. [Final observation index and shots](../../../../docs/internal/philo/phase-3/summary/round-2/integrated-observations-index.json), under `assets/story-02-shots/rig/round-2-integrated/`. Engine: **Qwen3.6-35B-A3B-UD-Q5_K_XL.gguf**, llama.cpp at `192.168.1.43:8080`. Astra inspected all fourteen integrated images and all fourteen pre-integration images.
- [Original 60 observations](../../../../docs/internal/philo/phase-3/summary/observations-index.md) remain immutable: 40 actual J4–J7 case-width pairs, 30 pass / 10 blocked, with both restart identity records. Six pre-integration and six integrated round-two runs bring the retained total to **72 observations**. No blocked/native case is promoted and no new round-two real-LAN J7 atlas restart walk is claimed.
- Post-push CI caught eight stale generated boundary line numbers. They are corrected with candidate contents unchanged; the complete twelve-command Documentation Navigation sequence passes in DW capture `2026-09-24T00:11:13Z`. [Failure and correction](../../../../docs/internal/philo/phase-3/summary/round-2/report.md#post-push-documentation-check).
...
LEDGER: [Aftercare covers Capture Bar verbs at 1440 and the lower summary/controls/fold at 393; ASR varies on the same WAV](../../../../docs/internal/philo/phase-3/summary/round-2/deferred-ledger.md): inherited **b**, ledgered only as instructed. [New planted-fact mapping](../../../../docs/internal/philo/phase-3/summary/round-2/usefulness.md): SQLite and Maya's identity are lost/corrupted; Leo's action/Tuesday survive; Priya's failure-fence action and before-ship timing are omitted at both widths. The integrated 393 ready observation retains failed raw summary hit ownership; technical state success does not claim whole-face clearance. Earlier inherited suite debt and graph-census debt remain named in their records.
