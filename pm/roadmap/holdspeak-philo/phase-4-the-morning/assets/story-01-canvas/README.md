...
**Status: UNRATIFIED, round four.** Astra checked round one and bounced it (six findings); round two paid them. Astra checked round two: RATIFY-WITH-CONDITIONS (four conditions); round three paid them. Astra checked round three: canvas RATIFY-WITH-CONDITIONS, library BOUNCE. Round four pays both. The owner has not seen these boards. Build nothing until he ratifies them (UX-CANON A.2).
...
4. **Found, not changed (for the orchestrator).** The producer sorts a section by priority (`_compose`, `:331-335`): `Review decision` (200) comes before `Commitment due` (110). Board 0 today shows `… Commitment due …, Review decision …` and says the decision is behind `1 more`; with the producer's order the decision is row 3 and the commitment is behind `1 more`. Boards 1, 2a and 3a show `Review decision, Meeting recorded, Open loop` (commitment folded); story 02's rule (decisions section first) gives `Review decision, Commitment due, Meeting recorded`. Boards 0, 1, 2a, 3a are unchanged in this round. Also: the `RouteDisclosure` chips inherit EgressChip's default tooltip `The route for this face is not set.` on a known destination (`facts.json` `lib-route-fallback-393`); not changed here.
...
1. **Board 7 split** (condition 1). The producer writes `Nothing material changed.` only when the brief has zero items (`holdspeak/services/monday_brief_service.py:340-341`). Triage writes the shelf (`shelve`, `:1110-1139`, into `monday_brief_item_shelf`); it does not rewrite the stored headline. So "empty" and "fully triaged" are two faces:
   ...
   - **7b, fully triaged.** The day-one brief (4 items), every row Ack'd or Defer'd. `untriagedBrief` is empty (`ChairHome.tsx:806-808`), so the quiet branch renders (`:1286-1328`): the rows leave the Arrival, the head label is `BRIEF` with no count, the **stored headline** shows (round four: `1 thing changed, 1 thing waiting, 2 decisions waiting.`, the real `_compose` output, `monday_brief_service.py:327-387`), then the caption. Today the verb is `Generate again`; proposed `Generate`. **Observation, not a proposal:** that headline still counts the four items the owner has triaged. It is a sentence, and its counts are stale after triage. The owner decides whether it stays.
...
| 7b | Fully triaged | `shots/7b-fully-triaged-1440.png` | `shots/7b-fully-triaged-393.png` | Day-one brief, all rows Ack'd/Defer'd: no rows, head `BRIEF`, the stored headline from the real `_compose` (`1 thing changed, 1 thing waiting, 2 decisions waiting.`; it still counts the four items: story 04), the caption, `Generate` (today `Generate again`) |
