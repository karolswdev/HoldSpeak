...
- [x] Shots at 1440 and 393 beside the canvas artboards. *(Product shots for boards 1, 3, 5, 6, 7 and 10 and for both refusal kinds; boards 2, 4 and 9 are fenced without a shot; board 8 has neither. The map: `assets/story-01-canvas/README.md`, "The canvas boards and the product shots".)*
...
- 2026-09-26 — round two of half B, the Astra-role check on PR #673 (`checks/story-01-halfb-astra-role-r1.md`, RATIFY-WITH-CONDITIONS): the Floor field now opens with the name selected, as the list does (ruled lawful: the one ratified behaviour). C1 the evidence names the half-A leg's `-n 6` failure as deterministic (the removed button), only the POST wait as load; C2 the failed-save glass runs on both faces (list and spatial); C3 BACKLOG rows for the clipped Floor field and its unbacked chip (F3), the refused name re-sent on each blur (F4) and the late receipt dropping the hub's reason (F5); the board-to-shot map in the canvas README.
