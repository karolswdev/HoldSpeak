...
- **Phase:** 15
- **Status:** done (2026-07-05 — **the runner dispatches per RUNS-ON**; see
  `evidence-story-04.md`. The closing slice landed with HSM-15-02's build: an injected
  `MeshDispatch` runs a `.dispatchToMac` step on the paired peer over `POST /api/ask`
  under the SAME bounded retry loop, per-step `targets` resolve each node's pin (empty =
  byte-identical legacy), `StepOutcome.ranOn` states where the output actually came from
  (a fallback reports `.onDevice` — it never left), and no-paired-peer rides the failure
  policy exactly like an unreachable endpoint. Proven end to end: the Simulator's pinned
  step landed on a REAL local hub (prompt receipts) and the HUD job settled labeled with
  the peer's name. Earlier: the pure runner host-proven 2026-06-22; the canvas execution
  via HSM-14-15.)
