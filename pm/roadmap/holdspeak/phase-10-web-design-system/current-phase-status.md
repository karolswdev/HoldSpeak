...
Through HS-10-07, the design-system foundation (tokens, components,
TopNav, identity, runtime dashboard) is in place and the densest
route — `/activity` — has been rebuilt onto it. The legacy 940-line
`activity.html` is gone; the page is generated by Astro from
`web/src/pages/activity.astro` + `web/src/scripts/activity-app.js`,
served from `holdspeak/static/_built/activity/index.html`. Preview-
vs-saved candidate grammar and the connector control pattern are
now token-driven and reusable for the remaining route rebuilds.

Up next: HS-10-08 (`/history`) and HS-10-09 (`/dictation`) follow the
same pattern, then HS-10-10–13 (CommandPreview, destructive
confirmation, motion/a11y, designer handoff refresh).
