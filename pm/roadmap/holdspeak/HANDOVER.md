...
- **Active phase:** **Phase 26 — Web Runtime Decomposition** (`in-progress`, 5/7 done). **Next story: HS-26-06** (collapse the `MeetingWebServer` constructor callback bag into `WebContext`; re-home the two still-shared helpers `_meeting_callback_payload` + `_parse_iso_datetime` so `meetings.py` + `activity.py` stop importing from `web_server`). Then **HS-26-07** closeout. All route domains are now extracted: `routes/{core,meetings,dictation,activity,pages,system,projects}.py`. **`web_server.py` is now a thin assembler — 532 lines (was 5658, −91%): only middleware + lifespan + StaticFiles mount + device-audio WS registration + `include_router` wiring.** Conventions honored throughout: WebContext param is **`ctx`** (shadowing locals are `project`); **relocate** single-domain helpers into the route module, import only genuinely-shared ones; watch `Path(__file__)` after moving a module (anchor to the package dir); run the **full** suite, not a narrow `-k`, as the gate.
