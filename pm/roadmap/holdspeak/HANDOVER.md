...
- **Active phase:** **Phase 26 — Web Runtime Decomposition** (`in-progress`, 4/7 done). **Next story: HS-26-05** (device / companion / project routes — plus the inline `/api/projects/{project_id}/briefings`, `/api/settings`, and the dashboard/page GET routes). Shipped: meeting/speaker/intel → `routes/meetings.py` (HS-26-02), dictation/agent-hook/intent → `routes/dictation.py` (HS-26-03), activity/connector/plugin-job → `routes/activity.py` (HS-26-04). **web_server.py is now 1817 lines (was 5658, −68%).** The §3 TL;DR below describes the HS-26-02 pickup — follow the same seam pattern. Conventions to honor: WebContext param is **`ctx`** in every route module (name shadowing locals `project`, not `ctx`); **relocate** helpers used by only one domain into that route module rather than importing from `web_server`, and import only genuinely-shared helpers (HS-26-06 re-homes those); run the **full** suite, not a narrow `-k`, as the gate.
