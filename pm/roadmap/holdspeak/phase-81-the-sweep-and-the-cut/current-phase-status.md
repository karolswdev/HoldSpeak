...
| HS-81-05 | Docs + harness + protocol truth (F-08, F-06, F-11) + the results addendum | done (ledger fully closed/re-scored) | HS-81-01..04 |
| HS-81-06 | The v0.4.0 cut prep (version, CHANGELOG roll, build + suite green) | done (wheel verified; doctor 0; suite green) | HS-81-01..05 |

## Where we are

CLOSED, opened and shipped in one sitting. Every dogfood small finding is
fixed (with tests where the finding was filed) or honestly re-scored, and the
v0.4.0 cut is fully prepped: version + CHANGELOG + install ref + verified
artifacts + a green gate. **Tagging `v0.4.0` publishes to PyPI (release.yml)
and is the owner's button** — nothing here pushed a tag.
