...
- [ ] `POST /api/dictation/dry-run` accepts `{utterance: str}`, returns the full pipeline-trace shape above.
- [ ] Empty / missing utterance returns 4xx with field-level detail.
- [ ] When the dictation pipeline is disabled in config, returns 200 with `runtime_status="disabled"` and an empty `stages: []` (matches CLI behaviour).
- [ ] UI panel renders the trace clearly; "copy final text" works.
- [ ] Integration tests cover the 4 paths above.
