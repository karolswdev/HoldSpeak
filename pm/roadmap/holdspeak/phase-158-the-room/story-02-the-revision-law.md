...
- **Follow-up chunk (found by the 05 glass rig):** the §5.1 room fields had no HTTP write path. `update_project` extended: 12 fields with typed validation (closed lifecycle `proposed|active|paused|complete|cancelled`, `archived` refused by name toward `archive_project`; owner_ref through `holdspeak.refs`; ISO dates; capped texts; cadence/modules shapes). Transitions deliberately UNENFORCED in P1 (P1a owns the lifecycle UX; documented in the test). Route was already a transparent pass-through — zero route changes; legacy GET payload untouched (room fields live in /room); zero pin changes. +48 tests (`test_project_room_fields_write.py`).
