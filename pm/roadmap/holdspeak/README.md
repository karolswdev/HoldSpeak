...
**Last updated:** 2026-06-01 (Phase 29 **2/5** — HS-29-02 shipped: two incident stubs flipped to real (`incident_timeline`, `runbook_delta`), each with a structured `/history` render. **Twelve real plugins now, two stubs** (the comms drafters). Verified live on `.43` Q6. Suite green at 2039 passed, 14 skipped).
**Current phase:** [phase-29-plugin-rollout-iii-and-docs](./phase-29-plugin-rollout-iii-and-docs/current-phase-status.md) — in-progress (2/5; HS-29-01/02 done). Next: HS-29-03 (comms → zero stubs), 04 (public docs), 05 (close). Software-only. Other open tracks: **Phase 25** (trust & hardening; HS-25-07 hardware-blocked); **Phase 24** (companion, 2/5; HS-24-03/04/05 hardware-gated); **Phase 15** (out-and-about, `not-started`, gated on Phase 25).
**Status:** Phase 29 active (HS-29-02 delivered; next HS-29-03 comms plugins).
