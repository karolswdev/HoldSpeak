...
**Last updated:** 2026-06-02 (HS-30-08 shipped — **Phase 30 (UI/UX overhaul) in-progress 8/9.** History + Activity + Companion are now Signal and the Workbench `--wb-*` shim is **deleted — zero legacy tokens repo-wide**; the whole product is Signal end-to-end. Only the a11y/motion/exit pass remains. Build green, backend 2062 passed. Design system derived with the vendored `ui-ux-pro-max` skill)....
**Current phase:** [phase-25-trust-and-hardening](./phase-25-trust-and-hardening/current-phase-status.md) — in-progress (HS-25-01..06 + 08 done; HS-25-07 hardware-blocked, author remote). Other open tracks: **Phase 30** (UI/UX overhaul → "Signal" dark identity, 8/9, in-progress); **Phase 24** (companion, 3/6; HS-24-01/02/06 done, HS-24-03/04/05 hardware-gated); **Phase 15** (out-and-about, `not-started`, gated on Phase 25). Next plugin work is the **frontier** (authoring guide / packs / actuators), not stubs — see Phase 29's `final-summary.md` §Handoff.
