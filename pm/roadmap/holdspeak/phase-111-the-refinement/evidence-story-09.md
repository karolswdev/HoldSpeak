...
- **Story:** HS-111-09 - Sprite and icon quality
- **Status:** done
- **Date:** 2026-08-02

## Proof

### Captured run — 2026-08-02T08:00:14Z

- **Command:** `bash -c cd web && npm run check && cd .. && uv run pytest -q tests/unit`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 269dfaa0fbecf0fda6a29450bbe4db95879f35b2
...
token gate: clean (37 allow-listed exceptions, all in use)

> holdspeak-web@0.0.1 guard:architecture
> node scripts/guard-architecture.mjs

React architecture guard passed (211 source files; zero framework residue).
...
Error: Not implemented: HTMLCanvasElement.prototype.getContext (without installing the canvas npm package)
    ...
    at WorldEngine.init (/Users/karol/dev/tools/HoldSpeak/web/src/desk/gl/engine.ts:209:7) undefined
Error: Not implemented: HTMLCanvasElement.prototype.getContext (without installing the canvas npm package)
    at module.exports (/Users/karol/dev/tools/HoldSpeak/web/node_modules/jsdom/lib/jsdom/browser/not-implemented.js:9:17)
    at HTMLCanvasElementImpl.getContext (/Users/karol/dev/tools/HoldSpeak/web/node_modules/jsdom/lib/jsdom/living/nodes/HTMLCanvasElement-impl.js:42:5)
    at HTMLCanvasElement.getContext (/Users/karol/dev/tools/HoldSpeak/web/node_modules/jsdom/lib/jsdom/living/generated/HTMLCanvasElement.js:131:58)
 ...
 Test Files  76 passed (76)
   ...
   Start at  02:00:29
   Duration  33.02s (transform 2.28s, setup 4.24s, import 14.55s, tests 14.27s, environment 23.94s)
...
../holdspeak/static/_built/assets/inter-latin-500-normal-BL9OpVg8.woff       
[PMO_EVIDENCE_OUTPUT_TRUNCATED]
```

Full run (untruncated, output read): exit 0 — web check **76 test
files / 438 tests**, tokens/gates/typecheck/build green; then
`uv run pytest -q tests/unit` **3450 passed** (the story has no
python surface — confirmed by the run; no sprite path is referenced
or asserted server-side).

## What shipped

The craft story — every sprite judged on the real desk at rendered
size, per the verified audit (.tmp/hs-111-09-audit.md: KEEP 9 ·
TOUCH-UP 6 · REGENERATE 3 · RETIRE 4 · NEW 3):

- **Integer rendering everywhere (the biggest win, zero credits)**:
  every system sprite was fractionally scaled (dock 32→24, menu
  16→14) — pixel art smeared no matter the source quality. Now
  natural == rendered at every site (measurements-after.json).
  The dock-speak "noise" resolved into its real grille at 1:1 —
  one planned regeneration cancelled as unnecessary.
- **The record orb**: the CSS gloss-gradient orb — the last macOS
  survivor on the desk — replaced by a 40×40 pixel-art recessed
  button with an orange lamp; the recording pulse is flat
  brightness steps, no glow; 7 stale glow allowlist entries deleted.
- **Four orphans RETIRED honestly**: the HS-110-02 gadget sprites
  (incl. the traffic-light-green maximize) and backdrop-tile lost
  to the SVG VerbGlyphs and CSS crosshatch that had already won;
  files + registry keys deleted, ICON-DISCIPLINE.md amended, and a
  new systemSprites.test.ts guards no-orphans in both directions.
- **The emoji era ends on the desk**: 🎙 → a new 16px handheld-mic
  sprite; 🤖/🖥️ avatars → the existing automaton/cartridge sprites
  via a new AgentAvatar component; dock dingbats → VerbGlyph SVG
  paths.
- **Regenerated art (8 subscription generations, ~zero credit
  cost)**: the brand mark is a crisp signal-orange speech bubble
  (was a shapeless splat — before/after 4x crops in assets); the
  search glyph is a real magnifier (was a dandelion); menu-bell
  recolored LOCALLY with PIL (edit_image bills 20-40 gens for a
  palette swap — deliberate zero-credit route).
- **The judgment sheet** rides in assets (JUDGMENT.md): before/after
  per asset, the banked spark-mark alternate, and one decision HELD
  for the owner (sprite-faced chat/handshake windows — a
  registry-wide window-glyph grammar change, not minted
  unilaterally).

## Live proof (real hub, :8765)

Assets in [assets/hs-111-09/](./assets/hs-111-09/):
`before-menu-mark-splat-4x.png` vs `close-el-menu-mark-4x.png`,
`before-record-orb-gloss-4x.png` vs
`close-desktop-06-record-orb-4x.png`, the integer-crisp dock and
menubar, and JUDGMENT.md. Full 39-shot set with 4x crops of every
changed asset reviewed (.tmp/hs-111-09-after/).

## Honest notes

- The shot rig created a temporary note so the orb mounts; all rig
  notes confirmed deleted (API count 0).
- The chat/handshake NEW-ART entries shipped as type-scale
  characters; the sprite-face grammar decision is held for the
  owner's sitting (documented in JUDGMENT.md).
