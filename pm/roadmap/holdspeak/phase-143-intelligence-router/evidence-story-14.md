# Evidence - HSEGHS001HS104-143-14

- **Story:** HSEGHS001HS104-143-14 - Chaos, Glass, and Closeout
- **Status:** in-progress
- **Date:** 2026-08-27

## S1 production-composition proof

`tests/integration/test_phase143_closeout_chaos.py::test_one_restart_cross_product_preserves_frozen_recipe_receipt_library_and_assignment_truth`
ships in `c55062e0ff89ef8ff41f8561ea2d091961a7e568`. It uses real SQLite, the production broker/application composition, Recipe chat, a durable Model Library command, and an assignment mutation across one fresh-composition restart. Only the local engine and download response are bounded external-wire adapters. It proves frozen Recipe identity/receipt reconstruction, immutable assignment-command replay, unchanged assignment-head bytes across the library replay, custody absence, hostile v2 sync refusal, and unknown dispatch terminality with no second egress. It intentionally does not use a literal SIGKILL harness; the durable database plus fresh production composition is the recovery boundary.

Fixture note: both qualified route legs are local. The v2 binding law lawfully refuses a fabricated cloud-shaped v1 fixture; this is not a claim about a cloud fallback in the chaos fixture. Saved local-to-cloud authorization remains separately proven by Story 06's production-adoption proof.

## Captured verification

### S3 final document, census, one-path, transport, chaos, and glass guards

- **Command:** `HOME=<isolated scratch HOME> PLAYWRIGHT_BROWSERS_PATH=<owner browser cache> npm_config_cache=<owner npm cache> uv run pytest -q tests/unit/test_doc_drift_guard.py tests/unit/test_phase143_inference_capability_census.py tests/unit/test_phase143_routing_authority_census.py tests/unit/test_phase143_surface_fallback_census.py tests/unit/test_one_path_census.py tests/unit/test_one_path_cardinality.py tests/unit/test_one_path_provenance.py tests/unit/test_one_path_spine.py tests/unit/test_api_surface.py tests/unit/test_phase143_transport_parity.py tests/integration/test_phase143_closeout_chaos.py tests/e2e/test_hs143_assignments_glass.py`
- **Source commit/tree before S2/S3 documentation:** `c55062e0ff89ef8ff41f8561ea2d091961a7e568` / `8f55eece1b31ebad78732c515bbf893e1c8ccd4a`
- **Exit code:** 0

```text
182 passed in 158.86s (0:02:38)
```

### Ledger-citation spot run

- **Command:** `HOME=<isolated scratch HOME> PLAYWRIGHT_BROWSERS_PATH=<owner browser cache> npm_config_cache=<owner npm cache> uv run pytest -q tests/unit/test_phase143_inference_route_plans.py tests/unit/test_phase143_inference_fallback_controller.py tests/unit/test_inference_runner.py tests/unit/test_phase143_production_adoption.py tests/unit/test_phase143_tool_turn_routing.py tests/unit/test_phase143_tool_turn_model_steps.py tests/integration/test_phase143_tool_turn_boundaries.py tests/integration/test_phase143_placement_adoption_matrix.py`
- **Source commit/tree before S2/S3 documentation:** `c55062e0ff89ef8ff41f8561ea2d091961a7e568` / `8f55eece1b31ebad78732c515bbf893e1c8ccd4a`
- **Exit code:** 0

```text
218 passed in 57.04s
```

The S3 command's shell postscript attempted to assign zsh's read-only `status` parameter after pytest had already completed. The recorded pytest output above is complete and green; the postscript's shell exit is not a test failure. The citation spot run used a non-reserved `rc` parameter and exited 0.

### S3 final-tree rerun after ledger and exit-box edits

- **Command:** the same isolated-HOME 12-file S3 guard command above, after the
  closeout ledger, this evidence record, and the phase-status exit mechanics
  existed in the worktree.
- **Exit code:** 0

```text
182 passed in 156.55s (0:02:36)
```

A direct citation check also passed: all 12 kill criteria match the architecture
contract verbatim after whitespace normalization; all 79 local Markdown links
and 16 cited test files resolve. No census/anchor inventory was edited.

### Captured run — 2026-08-27T02:42:49Z
...
- **Cwd:** .
- **Exit code:** 1
- **Index-tree:** 8f55eece1b31ebad78732c515bbf893e1c8ccd4a

```text

tests/unit/test_inference_setup_capability_truth.py:140: AssertionError
____________ test_primary_copy_has_no_prohibited_operational_drift _____________
[gw7] darwin -- Python 3.13.11 /Users/karol/dev/tools/HoldSpeak/.venv/bin/python3
...
[gw11] darwin -- Python 3.13.11 /Users/karol/dev/tools/HoldSpeak/.venv/bin/python3
...
SKIPPED [1] tests/integration/test_dictation_llama_cpp_e2e.py:72: llama-cpp-python and /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/tmp.rDudQ2xGQP/xdist-gw1/Models/gguf/Qwen3.5-4B-Instruct-Q4_K_M.gguf are required for this integration test
...
SKIPPED [1] tests/integration/test_runtime_llama_cpp.py:38: llama-cpp-python and /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/tmp.rDudQ2xGQP/xdist-gw2/Models/gguf/Qwen3.5-4B-Instruct-Q4_K_M.gguf are required for this integration test
SKIPPED [1] tests/integration/test_runtime_mlx.py:38: mlx-lm + outlines + /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/tmp.rDudQ2xGQP/xdist-gw2/Models/mlx/Qwen3.5-8B-MLX-4bit are required for this integration test
...
11 failed, 6680 passed, 53 skipped in 491.84s (0:08:11)
```

## Orchestrator triage note (2026-08-27)

The captured run exits 1 lawfully and is the cleanest of the phase:
6680 passed / 11 failed, and the 11 are the inherited baseline EXACTLY
— zero non-baseline names, no flakes fired, and the freshly-fixed
assignments-overview 393 leg held through full xdist (as it did in the
pre-capture sweep, where the only two non-baseline names were the
device-recording-tick and workbench-deadline known flake families, each
serial-green ×2). The exact-baseline close rule from the plan's
dispositions is met at its strictest reading. The write-once ledger
(`assets/phase-143-closeout-ledger.md`) carries the twelve kill
criteria and nine exit criteria with verified production-path
citations; all nine exit boxes are checked in the status doc. Phase 143
closes complete: 15/15 stories, every production inference call behind
one versioned capability with frozen plans and honest receipts, both
owner surfaces shipped and walked, transport parity proven, the router
documented repo-wide, and the phase's residue stated honestly — 11
inherited failures with named non-router ownership and seven Swift
leaves HELD on their frozen seed branch.
