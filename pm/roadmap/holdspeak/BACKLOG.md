...
| VERIFIED (`docs/internal/philo/phase-7/floor-diag/FINDING.md`, PR #667): at 393 the Floor opens as a list (`web/src/desk/store/types.ts:65-72`) and the list view's row-menu Delete (`web/src/desk/components/DeskListView.tsx:332`, `objectMenuEntries`) appears to have no listener; the delete listener exists only in the spatial view. Read from code by the lane; not exercised by the lane or the counsel. First step: exercise it on an isolated hub at 393; file a defect only if it reproduces. | suspected face defect (UNVERIFIED) | PHILO-7-03 evidence, face finding 3 | ASSIGNED: Muad'Dib (the Arrival/Floor lane) — PAID by PHILO-8-02 (`docs/internal/philo/phase-8/one-delete/README.md`) |

## The Floor delete receipt follow-ups — 2026-09-25 evening (Astra role, Opus 5.5 stand-in, on PR #665)

| Item | Kind | Source | Home |
|---|---|---|---|
| Two Floor deletes inside one undo window silently keep the second: delete decision A, wait 1.5 s, delete decision B, wait 12 s → A 404, B 200 (B is still on the server while the owner saw it removed). On main too. Likely cause: `remove()` calls `cleanup()`, which drops the earlier pending action (`web/src/desk/hooks/useUndoReceipt.ts:27-29`), and the Floor's undo step does nothing (`web/src/desk/gl/WorldStage.tsx:144-148`); unverified which one is lost and why. A second delete must commit or queue the first, never drop it; a fence with two deletes in one window. | a delete that does not happen while the face says it did (Tenet 3) | `checks/delete-receipt-astra-role-r1.md` finding 7 | ASSIGNED: Muad'Dib, the Floor lane (with the rename-field and list-view Delete defects, `docs/internal/philo/phase-7/floor-diag/FINDING.md`) — PAID by PHILO-8-02 (`docs/internal/philo/phase-8/one-delete/README.md`) |
...
| The Floor's zone name field is clipped: its width is the zone label's (`web/src/desk/gl/WorldStage.tsx`, `width={renameZone.width}`), not the row's, so the text is cut at both widths (`ıe zone` for `Offline zone` at 393, about 62 px wide; `Inbox` in about 60 px at 1440); and the Floor's refusal chip has no backing, so the sprites show through it, and it is about 8 px wider than the field and mic at 1440. It is now the same element as the ratified list field (`web/src/desk/components/ZoneRenameRow.tsx`); the canvas drew only the list. | face | `pm/roadmap/holdspeak-philo/phase-8-the-honest-floor/checks/story-01-halfb-astra-role-r1.md` F3; shots `pm/roadmap/holdspeak-philo/phase-8-the-honest-floor/assets/story-01-shots/floor-name-taken-{1440,393}.png`, `floor-not-saved-network-393.png` | the Floor face pass; a canvas for the Floor field first |
| After a refused rename, every blur out of the field sends the SAME refused name again (`web/src/desk/hooks/useZoneRenameField.ts` compares to `title`, which was reverted) and each 409 takes the focus back: one PUT per click elsewhere until Escape. Inherited from main's hook; read from the code, not observed. | behaviour | `checks/story-01-halfb-astra-role-r1.md` F4 | UNASSIGNED |
| A late refusal (the field already closed) of a 422 or a network failure reads `RENAME ZONE FAILED · NOT SAVED` on the write receipt: the hub's reason (kept in `detail`) is dropped (`web/src/desk/store/dataSlice.ts` `renameZone`, `web/src/desk/DeskApp.tsx` pass `label`). | words | `checks/story-01-halfb-astra-role-r1.md` F5 | UNASSIGNED |
...
## PHILO-8-02 follow-ups — 2026-09-26 (the Fedaykin lane, `docs/internal/philo/phase-8/one-delete/README.md`)
