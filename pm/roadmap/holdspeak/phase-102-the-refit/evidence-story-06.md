...
- **Story:** HS-102-06 - Speech Settings — one composed face
- **Status:** done
- **Date:** 2026-07-22

## Proof

### Captured run — 2026-07-23T00:10:02Z
...
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** e3fdc224ffd579b4cec5b653f46cc359fc5edceb
...
Error: Not implemented: HTMLCanvasElement.prototype.getContext (without installing the canvas npm package)
 ...
 Test Files  49 passed (49)
   ...
   Start at  18:10:06
   Duration  6.26s (transform 3.65s, setup 4.48s, import 14.24s, tests 10.80s, environment 28.80s)
...
../holdspeak/static/_built/assets/webworkerAll-DL1WdeK1.js                              0.16 kB │ gzip:   0.16 kB │ map:     0.66 kB
../holdspeak/static/_built/assets/browserAll-BpYZD_5A.js                                0.25 kB │ gzip:   0.21 kB │ map:     1.67 kB
../holdspeak/static/_built/assets/WelcomePage-QgNeWFi8.js                               0.43 kB │ gzip:   0.31 kB │ map:     1.13 kB
../holdspeak/static/_built/assets/PresencePage-DsOgirwx.js                              0.89 kB │ gzip:   0.52 kB │ map:     2.46 kB
../holdspeak/static/_built/assets/pageSupport-DYk_rsKU.js                               1.01 kB │ gzip:   0.61 kB │ map:     5.63 kB
...
The owner's words: "The 'Speech Settings' is an absolute joke — a
cacophony of tiles, panes, forms, form groups and so on." The
2026-07-21 Design direction session (recorded in `story-06-speech-settings.md`)
had already convicted the gear face line by line against a fresh
headed drive: raw wire dumps ("PIPELINE READINESS — pipeline enabled
false / max total latency ms 600 / backend auto"; "RESOLVED DELIVERY
— id browser / confidence 0.78 / …"), two disconnected warning boxes
with no remedy, PROJECT SCOPE as a bare input + button, Knowledge and
Instructions as two giant textareas each with an orange Save button
(canon rule 1 violation, and — discovered mid-build below — never
actually wired to the real API shape), and DICTATION RUNTIME
re-stating Backend/Runs on/Latency budget as a third label-over-Select
stack duplicating `RuntimeDestination`.

## The fix

- **`Readiness()`** — `PIPELINE READINESS`/`RESOLVED DELIVERY` compose
  into a `SurfaceGroup`/`SurfaceSettingRow` "Types automatically as you
  speak · auto · budget 600 ms" line with the state as a live
  `SurfaceToggle` (flips `dictation.pipeline.enabled` via the same
  `/api/settings` PUT), a composed "Last typed into `<target>` via the
  browser bridge · NN% confidence" delivery line, and a `missing_project_kb`
  warning gets an inline "Create it" verb (`POST
  /api/dictation/project-kb/starter`) — its own row, not a floating
  banner. Every other warning renders as one composed sentence (no
  box). Raw `config`/`target`/`depth` fields move behind a `Disclosure`
  ("Wire details"), off by default.
- **`LearningDigestFacts()` (new)** — `SurfaceFacts` on the raw
  `/api/dictation/learning-digest` payload was accidentally showing
  ONLY its metadata wrapper (`window`/`enabled`/`generated_at` — the
  one top-level scalars — while the real counts live nested under
  `totals` and were silently dropped by `SurfaceFacts`'s object
  filter). Composed into "N corrections taught this week · N
  dictations corrected · reached N similar" plus the top 3
  `by_block` breakdown. A real defect this recompose caught, not
  invented scope.
- **`Knowledge()`** — Project scope is one `SurfaceSettingRow`
  (`EditInPlace` root text + "Use project" verb). Knowledge and
  Instructions no longer save through the wrong wire shape: the OLD
  code PUT `{content: text}}` to both `/api/dictation/project-kb` and
  `/api/dictation/project-hs`, but those routes have ALWAYS required
  `{kb: {<KEY>: <value>}}` (project-kb is a validated facts glossary,
  `[A-Za-z_][A-Za-z0-9_]*` keys, string-or-null values — not free
  text) and `{files: {<name>: <content>}}` (project-hs is multiple
  named `.hs/*.md` files) respectively — every prior "Save
  knowledge"/"Save instructions" click has always 400'd. Knowledge is
  now a `SurfaceRows` list, one row per fact, each value `EditInPlace`,
  with a small "+ Add fact" composer (name + value + verb, validated
  against the same key pattern the backend enforces) and a per-row
  "Forget" `ConfirmVerb`. Instructions binds to the primary
  `.hs/instructions.md` file, `EditInPlace` multiline, saved via the
  correct `{files: {...}}` shape. Both orange Save buttons are gone;
  saves happen on commit with a quiet "Saving…/Saved" whisper
  (`settings-save-whisper`, the exact class Settings' own configuring
  archetype already uses).
- **`EditInPlace` (`Surface.tsx`, shared kit)** — gained
  `onFocus={(e) => e.target.select()}`. Placeholder-shaped values
  ("No instructions yet — click to add") ride the same `value` prop
  as real content; without a select-on-focus, typing appended after
  the placeholder instead of replacing it (caught live: the first
  drive produced "Always cite the ticket number in commit
  messages.No instructions yet — click to add."). A backward-compatible
  fix to the shared component — every other `EditInPlace` call site in
  the app benefits, none regress (full vitest re-run green after the
  change).
- **`Runtime()`** — the Backend/Runs on/Latency budget `Field`/`Select`
  triple is gone; the group now embeds `RuntimeDestination`
  (`settingsBespoke.tsx`), the SAME component Settings uses for this
  exact `dictation.runtime` value — one owner, not a third copy.
  Saves debounced-on-change (700 ms, the SettingsCore pattern),
  whispering "Saving…/Saved" in the section header instead of a
  "Save runtime" button.

## Driven live, after (headed, staged instance, both viewports)

Staged via `uv run python -m uat.stage --recipe seeded-desk` (real
seeded desk + a real detected project). At 1440 and 393: opened the
gear door and confirmed the whole face reads as composed groups (no
raw keys anywhere outside the Wire-details disclosure); created a
starter Project KB via the inline "Create it" verb (warning cleared on
reload); added a real fact (`BLUEBIRD → the codename for the
ledgerline rewrite`) through the new facts list and confirmed it
persisted across a fresh page load; edited Instructions in place
(`.hs/instructions.md`) and confirmed the saved text was exactly what
was typed (post select-on-focus fix — the pre-fix drive had caught the
placeholder-append defect, screenshotted, then fixed and re-verified);
switched the Dictation runtime bay to "This device · llama.cpp" and
back; toggled the pipeline on/off and watched the Delivery/Learning
digest lines update live. Screenshots:
`assets/hs-102-06-before-pipeline-kb-warning.png` (Pipeline off + the
KB warning with its inline "Create it" verb),
`assets/hs-102-06-knowledge-fact-added.png` (the facts list + composer,
BLUEBIRD saved), `assets/hs-102-06-wire-details-disclosure.png` (raw
fields behind the Disclosure, off by default),
`assets/hs-102-06-after-full-face.png` (the whole composed face,
maximized, Instructions correctly showing only the typed sentence),
`assets/hs-102-06-mobile-393-configure.png` (393, single-column
reflow, same groups).

## Walk legs / guards

No new interior-canon-guard-grade defect pattern needed a fresh smoke
walk leg (this story's regression surface is a single core file); the
guard added below pins it directly. Full vitest (312/312 — including
every other `EditInPlace` call site, unaffected by the shared-kit
change), tsc, build, token gate, interior-canon + vocabulary guards
all green.

## Guard added

`test_dictation_core_speech_settings_never_regresses`
(`tests/unit/test_interior_canon_guard.py`) — pins zero "Save
knowledge"/"Save instructions" buttons, the presence of the shared
`RuntimeDestination` embed, and `SurfaceGroup`/`SurfaceToggle` usage
in `DictationCore.tsx`, named by story so a future "quick fix" can't
quietly bring the old form stack back.
