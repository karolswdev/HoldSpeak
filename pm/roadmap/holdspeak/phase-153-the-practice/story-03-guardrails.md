...
- **`run_guardrail` untestable through the real coordinator** (HS-153-05 fix) -- same three defects as `run_compact`: `deadline_at=0`, `deployment_revision=""`, wrong adapter shape, missing publish callback. Fixed in story 05; two mocked tests in `TestRealCoordinatorGuardrail` now drive through the real runner.
